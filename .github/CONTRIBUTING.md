...
`main` may require signed commits via branch protection (see
[CI/CD Guide -- Branch protection](../docs/cicd.md#branch-protection-recommended-settings)).
If signing is enforced, configure git locally so every commit is verified
by GitHub:

- **GPG**: follow GitHub's
  [signing commits guide](https://docs.github.com/en/authentication/managing-commit-signature-verification/signing-commits)
  and set `git config --global commit.gpgsign true`.
- **SSH**: upload an SSH key as a signing key on your GitHub account,
  then set `git config --global gpg.format ssh` and
...
`release.yml` runs as `github-actions[bot]`, which cannot satisfy
"require signed commits" on its own. Either add the bot to the ruleset
bypass list or switch the release identity to a PAT / deploy key with a
