...
5. Hybrid storage model (BigQuery + Lakehouse/Iceberg)

When to split diagrams:

- **Dataflow (`1` vs `2`)**: use one when stream and batch/replay share most
  transforms; use two when windowing, dedup, contracts, or sinks differ materially.
- **Cloud Functions (`1` vs `2`)**: use one when ingestion and trigger handlers are
  simple and coupled; use two when they are independent deploy/ownership units.
