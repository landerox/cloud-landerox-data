...
class KappaOptions(PipelineOptions):
    ...
    def _add_argparse_args(cls, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--kms_key",
            help="Cloud KMS key for encrypting the output table (CMEK)",
        )
...
def get_source(options: KappaOptions) -> beam.PTransform:
    ...
    if options.input_mode == "stream":
        if not options.input_subscription:
            raise ConfigError("--input_subscription is required for stream mode")
    ...
    if not options.input_path:
        raise ConfigError("--input_path is required for batch mode")
...
def get_sink(
    ...
    The ``schema`` argument accepts the two static shapes documented for
    ``WriteToBigQuery``: a legacy ``"field:TYPE,..."`` string or a
    ``TableSchema``-shaped dict. Callable / dynamic-destination schemas are
    out of scope for this baseline; pipelines that need them should
    instantiate ``WriteToBigQuery`` directly.
    """
